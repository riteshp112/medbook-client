...
export const loadPost = (setPostItems,setModalVisible,PostItems) => {
  ...
  medFetch({type:'select',table:'post',condition:{} ,limit: "" + (PostItems.length+5)}).then(response=>response.json()).then(json=>{posts=json.response
    if (posts.length > PostItems.length){
      for (let item of posts)     
        newPostItems.push([<PostItem key={item?._id} item={item} PostItems={PostItems} setPostItems={setPostItems} setModalVisible={setModalVisible}></PostItem>])
